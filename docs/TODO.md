...
- [in-progress] **11.8** **Frontend — shared-api + atoms** (Slice 7): `getTenantId`/`setTenantId` cookie helpers in `packages/shared-api/src/index.ts` (mirror `getToken`); `authedFetch` injects `X-Tenant-Id`; `tenant:changed` CustomEvent; `listTenants`/`createTenant`/`renameTenant` in `packages/app/src/api/chat.ts`; `tenantsAtom/View` + `currentTenantAtom` in `packages/app/src/atoms.ts`; reactivity keys tenant-aware (`sessions:<tenantId>`, `turns:<tenantId>:<sessionId>`).
