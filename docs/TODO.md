...
- [in-progress] **0.1** Draft `docs/SPEC.md` with three actors, principles, seed stories S1–S7, synergies, dynamic, isolation, trace model, open questions.
- [in-progress] **0.1b** Deduce and consolidate Tier 0–3 Laws into §3 of SPEC. Ratify Tier 1 (constitutional) before Phase 1 begins; Tier 2/3 may continue iterating into Phase 1.
- [in-progress] **0.1c** Risk Register (§3.5) — **ship bootstrap thresholds** under `bootstrap=true` (L3.8). Do not negotiate values upfront. Phase 1 enforces R1, R2, R5 in the Supervisor; the rest are watched manually until evidence accumulates.
- [in-progress] **0.1d** Roles & Fitness (§4) — **ship bootstrap defaults** under `bootstrap=true` (L3.8): 3 seed roles (Architect, Implementer, Reviewer), 6 fitness dimensions, N=3 variants per goal, ~15 % diversity reserve. Refinement comes from the variant log after first goals run, not from agreement now.
...
- [done] **0.3** Resolve enough of §10 open questions to start Phase 1.
  ...
  - Q3 (working-methods storage) — deferred; bootstrap is "subdirectory in managed workspace under `methods/`" until evidence demands otherwise.
  - Q2, Q5, Q7, Q8 — deferred; discover-and-adapt (L3.8).

**Exit:** Tier 1 Laws ratified; bootstrap values shipped in SPEC (Risk Register §3.5, fitness vector §4.4, role catalogue §4.1, variant/diversity defaults §4.5, budgets §4.6) — all flagged `bootstrap=true` per L3.8. Phase 1 begins; **calibration continues in-flight from operational evidence**, not from further negotiation. Phase 0 is "done enough to start measuring," not "done."
...
- [todo] **1.1** Host package layout (`packages/host`) with `packages/host/src/ports/` and `packages/host/src/adapters/`. **Dep-cruiser rule:** non-adapter code cannot import from `adapters/` (enforces L2.14). Service registry at `packages/host/src/runtime/bind.ts` wires ports → adapters at boot and emits `AdapterBound` events.
- [todo] **1.2** **`ObservabilityGateway` port** + **stdio MCP adapter** (the Outer MCP Claude attaches to). Connect + list-events + replay at first; promote/rollback wired in Phase 4.
- [todo] **1.3** **`UserGateway` port** + **`CliUserGateway` adapter** (`bin/user` talking to a local HTTP endpoint). One User per CLI invocation in v0. Distinct from 1.2 — see §10.1 Q4.
- [todo] **1.4** **`LlmProvider` port** + **OpenAI-compatible HTTP adapter**, env-configured base URL. Default target: LMStudio on host machine. Model id and seed (when available) captured in events for L3.6.
- [todo] **1.5** **`WorkspaceMount` port** + **mounted-git-repo adapter**: separate git repo path injected via env; Host validates on boot and refuses to start if the boundary is ambiguous.
- [todo] **1.6** **`EventStore` port** + **JSONL-on-disk adapter** (schema per §9, event kinds enumerated there). `storyRef` field present from day one.
- [todo] **1.7** **`DataHandle` port** + **`FileBackedHandle` adapter** (§10.1 Q6). `AggregateResult` shape (exitCode, stdoutHash, summary, bitsConsumed) is the only return surface. Includes `revoke`, `isAlive`, lifecycle events (`HandleRegistered`, `HandleRevoked`, `HandleExpired`, `HandleExhausted`). Unlocks S1.
