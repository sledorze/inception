...
- [in-progress] **11.9** **Frontend — switcher UI** (Slice 8): tenant switcher dropdown in `packages/app/src/components/app/AppShell.tsx` header (list entitled tenants + create + rename). Component test (`*.test.tsx`). On switch: set cookie → dispatch `tenant:changed` → navigate `/` → invalidate session/turns atoms.
