...
- **L0.3 — Asymmetry Disclosure.** Where actors hold asymmetric privileges, the asymmetry is named here and visible in the trace. Hidden asymmetry is forbidden.
  **Named asymmetries (Tier-1):** (a) `admin` = the constitutional system administrator / User-of-record (L0.2); holds read/write authority over the oversight loop, proposals, quarantine, and event trace via the `AdminQuery` driving port. (b) `enduser` = User (§1); holds authority over goal submission and conversation via the `UserGateway` driving port only. No admin code is reachable from the end-user bundle (structural isolation at build boundary). (c) The asymmetry is disclosed in the event trace: every successful login emits an `Authenticated` event with payload `{subject, role}` — NO token, NO password — making the privilege level queryable by the out-of-domain Monitor (L3.7). (d) The security claim is **structural** at the Vite bundle boundary (no admin code in end-user build) and **aspirational** at the in-process RBAC layer (shared trust domain within the Host process — a known limitation per Hardy/Reason).
  _Derives from:_ §2.4. _Enforced by:_ `AuthGateway` driving port (`packages/host/src/ports/driving/AuthGateway.ts`) + `requireRole` application service (`authorize.ts`) enforces RBAC at every `/api/*` endpoint; `Authenticated` event in `EventStore` (kind `Authenticated`, payload `{subject, role}`) makes asymmetry trace-visible; `AdminQuery` driving port (`AdminQuery.ts`) scopes admin-read access; separate Vite builds for `packages/backoffice` and `packages/app` isolate admin code structurally. Parametrised protocol test: `tests/protocol/AuthGateway.spec.ts` (Fake + Scrypt). Law test: `tests/laws/L0.3.spec.ts`. _If absent:_ Users cannot reason about who controls outcomes; hidden admin routes leak internal state (L0.3 violation); enduser bundle could contain admin code.
...
- **L1.9 — Tenant Isolation.** Every persisted event and every query is scoped to exactly one tenant. A principal may only access tenants it is entitled to; the Host verifies entitlement on every request before binding the tenant context. Cross-tenant read or write is forbidden; a missing or un-entitled tenant yields 403, not a narrowed scope.
  _Derives from:_ §8 (Isolation Guarantees); multi-tenancy security principle (no shared namespace across principals of different trust contexts); Evans (bounded-context isolation — conversations belonging to tenant A are not part of tenant B's bounded context). _Enforced by:_ `tenantId` is a required field on `StoredEvent`/`NewEvent`; `EventStoreQuery.tenantId` filters at the storage/query layer; `withTenant` HTTP middleware reads the active tenant from the `X-Tenant-Id` request header, validates the authenticated principal's `AuthIdentity.tenantIds` entitlement, and refuses with 403 on mismatch; `CurrentTenantId` FiberRef (sibling of `CurrentCorrelationId` in `domain/tracing.ts`) propagates the authorized tenant through the Effect context so all downstream appends — including toolkit events emitted under `sessionId='bootstrap'` — carry the real `tenantId`; EventStore protocol test (`tests/protocol/EventStore.spec.ts`) parametrised over InMemory + SQLite proves the tenant-filter contract under both adapters. _If absent:_ any authenticated principal can read or write any other principal's conversations; `listSessions` leaks the full event store to every caller; the isolation guarantee is structural in name only.
...
_Experts:_ Cockburn (extend driving `UserGateway` port — the seam, don't bolt routes on); Evans ("Project" is SPEC `Session` — no new vocabulary; **see S12 amendment**); Kleppmann/Young (multi-turn = projection over `sessionId` chain, `schemaV`, no event reshaping); Russell (don't auto-close on "done" — L3.2 explicit); Beck (spike unknowns; refactor commits separate from feature).
_Build vs adopt:_ Reuse `Session`, `EventStore.query({sessionId})`, `CurrentCorrelationId`, `submitGoal`, `UserGateway`, `OpenAiCompatLlmProvider`; adopt VCR/cassette pattern as a thin Effect adapter for LLM record-replay. No new Project/Conversation aggregate (AL.7). _**S12 amendment (2026-05-18):** the "no new aggregate / no new vocabulary" ruling is deliberately superseded for tenant isolation — see S12 and L1.9._
...
#### S12 — Tenant-scoped conversations: User isolates sessions into projects and switches the active tenant

_Deliberately supersedes S6's "Project is Session / no new aggregate" ruling (L1.9, 2026-05-18). The Evans / AL.7 restriction in S6 is amended: `Tenant` is introduced as a bounded-context isolation boundary, not merely an organisational tag._

_Assessment frame (L3.9):_
_Success:_ Playwright e2e (`LLM_MODE=replay`): log in → Default tenant visible → create "Acme" → switch active tenant (cookie) → start a conversation scoped to Acme → switch back to Default → Acme conversation NOT visible (cross-tenant invisibility asserted end-to-end). `correlationId` provably maps to the turn. Law test `L1.9.spec.ts` green; `if absent` variant red without tenant scoping.
_Concern tag:_ §14 Security & threat modelling (tenant boundary = trust boundary; STRIDE cross-tenant read); §14 Hexagonal / DDD (Tenant = new bounded context with ubiquitous language).
_Experts:_ Evans (bounded-context isolation — Acme's conversations are outside Default's model; "Tenant" enters the ubiquitous language); Kleppmann (event-store column `tenant_id` + application-layer query scoping; SQLite has no native RLS — compensating control is the L1.9 law + query filter + law test); Cockburn (`withTenant` middleware extends the driving `AuthGateway` seam — not bolted onto routes).
_Build vs adopt:_ Reuse `EventStore` port + SQLite adapter (idempotent `ALTER TABLE ADD COLUMN tenant_id TEXT` + backfill); extend `ScryptAuthGateway` identity record with `tenantIds: string[]`; add `CurrentTenantId` FiberRef sibling to `CurrentCorrelationId`; adopt cookie-based active-tenant selection (mirror `getToken`/`setToken` in `shared-api`); `authedFetch` injects `X-Tenant-Id` header at the single HTTP chokepoint. No new storage engine; no new MCP protocol.
_Risk:_ toolkit events hardcoded to `sessionId:'bootstrap'` must carry real `tenantId` — mitigated by FiberRef propagation (not column-only); SQLite has no native RLS — mitigated by L1.9 + query filter + law test (parked P.6: Postgres exit plan). `tenantId` must NOT be in `computeContentHash` — historical rows are backfilled, so hash exclusion is load-bearing.
_Measurement:_ e2e cross-tenant-invisibility assertion green; `L1.9.spec.ts` green + `if absent` red; EventStore protocol test tenant-filter cases over both adapters green; 403 integration test for un-entitled tenant green.

- **Trigger:** User wants to separate personal experiments from a client project ("Acme"), or an admin wants to give a team member access to only one project's conversation history.
- **Actors:** User (creates/renames tenant, switches active); Host (`withTenant` middleware enforces; `CurrentTenantId` FiberRef propagates); Claude (outer observer — multi-tenant trace is scoped per tenant).
- **Exchange:**
  1. Host seeds a `default` tenant on first boot (idempotent); every seeded identity is entitled to `['default']`.
  2. User creates tenant "Acme" via `POST /api/tenants`; Host emits `TenantCreated` on the reserved `__tenants__` stream; grants creator entitlement.
  3. User switches to "Acme" — client stores `tenantId=acme` cookie; all subsequent requests send `X-Tenant-Id: acme` header.
  4. `withTenant` middleware reads the header, verifies `authIdentity.tenantIds.includes('acme')`, binds `CurrentTenantId` FiberRef; refuses with **403** if the principal is not entitled.
  5. Every `GoalSubmitted`/`GoalCompleted`/`ClarifyRequested` append carries `tenantId` from the FiberRef — including toolkit events emitted under `sessionId='bootstrap'`.
  6. `listSessions` and `GET /api/sessions/:id/turns` filter by `EventStoreQuery.tenantId`; Acme sessions are invisible under Default and vice versa.
  7. User switches back to Default — Acme conversations absent from the session list.
- **Code-over-data:** L1.3/L1.7 enforcement is per-session; tenant scoping is orthogonal and additive.
- **Goal advance:** User can separate client work from personal experiments; an admin can onboard a team member to only their tenant; Claude traces are hard-scoped and queryable per tenant.
- **Failure modes:** un-entitled tenant header → 403 before any data layer is touched; missing header → falls back to `default` if the principal is entitled (configurable) or 400; tenant not found → 404 from `withTenant`; `tenantId` missing on a legacy row → backfill default on migration reads.
...
| Tenant         | Every event and query is scoped to the requesting principal's active tenant (L1.9). A principal may only read or write events whose `tenantId` matches their `AuthIdentity.tenantIds`; `withTenant` middleware enforces this on every request and binds `CurrentTenantId`; cross-tenant access yields **403** before any data layer is touched.       |
...
tenantId      owning tenant; required on every event; NOT part of contentHash (access-control metadata — excluded for the same reason as `id`/`occurredAt`; including it would break re-verification of backfilled historical rows)
```
...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Language                               | TypeScript (Node) Host; TypeScript + React (`packages/app` end-user SPA, `packages/backoffice` admin SPA)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Strong type system; wide adoption; shared toolchain across packages                                                                                                                                                                                                                         | Low (Node ecosystem)                                                                                                                                                                                                                       | Per-package rewrite if a slice outgrows the runtime                                                                                                                                                                                              |
...
| `tenantId` excluded from `contentHash` | `contentHash = sha-256(kind ‖ schemaV ‖ canonicalJson(payload) ‖ correlationId ‖ sessionId ‖ storyRef)` — `tenantId` is **not** a hash input.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Tenancy is access-control metadata (same class as `id`/`occurredAt`); historical rows are backfilled to `tenant_id='default'` during migration, so including tenantId would invalidate all pre-migration chains; the hash proves _content integrity_, not access scope.                     | None — the hash formula is stable; retroactively adding tenantId would require re-computing every stored hash (breaks L1.4 tamper-evidence).                                                                                               | Introduce a separate `scopeHash` field if cross-tenant content deduplication is ever needed; never alter `contentHash`.                                                                                                                          |
| SQLite tenant migration strategy       | Idempotent init-time migration in `SqliteEventStore`: `PRAGMA table_info(events)` → if `tenant_id` absent: `ALTER TABLE events ADD COLUMN tenant_id TEXT` → `UPDATE events SET tenant_id='default' WHERE tenant_id IS NULL` → `CREATE INDEX IF NOT EXISTS idx_events_tenant_id`. Runs on every boot; safe on fresh and existing `data/events.db`. `InMemoryEventStore` supports `tenantId` natively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `ALTER TABLE ADD COLUMN` is the idempotent SQLite extension path; no daemon, no downtime, no schema version bump. SQLite has **no native row-level security**; the L1.9 compensating control is application-layer query scoping + law test.                                                 | Low — one-way column add; no existing constraint changed.                                                                                                                                                                                  | Postgres `EventStore` adapter with `CREATE POLICY tenant_isolation ON events USING (tenant_id = current_setting('app.tenant_id'))`. Tracked as parked item P.6.                                                                                  |
| `CurrentTenantId` FiberRef             | `CurrentTenantId: FiberRef<string>` in `packages/host/src/domain/tracing.ts`, sibling of `CurrentCorrelationId`. Bound by `withTenant` middleware on every authenticated request; all downstream appends — including toolkit events under `sessionId:'bootstrap'` — read `tenantId` from the FiberRef.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | A column-only approach cannot reach toolkit-internal appends (hardcoded `sessionId:'bootstrap'`); FiberRef propagation covers the full append graph without changing every call site.                                                                                                       | Low — FiberRef is the standard Effect context propagation primitive.                                                                                                                                                                       | Forward the value as an internal HTTP header when the host goes multi-process; re-bind on the receiving side.                                                                                                                                    |
| Active-tenant HTTP convention          | Client sends `X-Tenant-Id: <tenantId>` on every request; value is read from a cookie (`tenantId=<id>`). `withTenant` middleware reads the header, authorises against `AuthIdentity.tenantIds`, binds `CurrentTenantId`. Missing header + entitled principal → falls back to `'default'`. Un-entitled → 403. `authedFetch` in `packages/shared-api` injects the header at the single HTTP chokepoint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Standard multi-tenant HTTP convention (GitHub, Stripe, Shopify); cookie persistence survives page reloads; single-chokepoint injection prevents omission.                                                                                                                                   | Low — header name is internal; rename = search-replace in `withTenant` + `authedFetch`.                                                                                                                                                    | If subdomains-per-tenant are added, infer tenant from hostname and retire the header; `withTenant` is the only change point.                                                                                                                     |
...
- **Tenant.** An isolation boundary grouping conversations (sessions) under a principal's control (L1.9, S12). A principal may be entitled to multiple tenants; one is active per request (selected via `X-Tenant-Id` header, persisted as a cookie). All events, sessions, and turns are scoped to exactly one tenant at the storage layer. The UI vocabulary is "project"; the domain/code vocabulary is `tenant` / `tenantId`. Not to be confused with `Session` (the container for a single user interaction) — a tenant contains many sessions.
