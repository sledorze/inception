# Reusable Configurations as GitHub Gists

This repository includes **standalone configurations** that can be used without cloning the entire repo. All patterns are available via public GitHub Gist URLs.
...
## Why Use Gist URLs Instead of Local Files?

1. **No repo clone needed** - Just copy the URL from docs or search GitHub
2. **Versioned separately** - Can update gist without changing repo commit history
3. **Searchable on GitHub** - Users find them via search even if they never clone the template
4. **Reusable across repos** - Other projects can cite these as "proven patterns"
5. **Smaller repo footprint** - No need to ship every possible config variant
6. **No authentication required** - Anyone can read and copy configurations
