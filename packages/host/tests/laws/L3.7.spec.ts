...
layer(testLayer)('L3.7 — Continuous Risk Supervision', it => {
  it.effect('R1 trips when a HandleExhausted event is present (bootstrap DP proxy)', () =>
    Effect.gen(function* () {
  ...
  it.effect('R2 trips when uncorroborated-event ratio exceeds 5 % (L3.7, bootstrap=true)', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const store = yield* EventStore
  ...
  it.effect('R2 does NOT trip when all Georges events are corroborated', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const store = yield* EventStore
      for (let i = 0; i < 5; i++) {
  ...
  it.effect('R5 trips when a SandboxEscapeAttempt event is present (L3.7, threshold > 0)', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const store = yield* EventStore
  ...
  it.effect('tripped signals emit SupervisorTrip events (L3.7)', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const store = yield* EventStore
      yield* store.append(makeEvent(sessionId, 'host', randomUUID(), 'SandboxEscapeAttempt'))
  ...
  it.effect('SupervisorDivergence is emitted when Monitor disagrees with Supervisor (L3.7, L3.10)', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const supervisor = yield* Supervisor
      ...
      const divergences = yield* checkSupervisorDivergence(sessionId, supervisorResults, monitorResults)
      expect(divergences).toContain('R5')
    }),
  )

  it.effect('SupervisorDivergence event is stored in EventStore on disagreement', () =>
    Effect.gen(function* () {
      const sessionId = randomUUID()
      const store = yield* EventStore
      const supervisor = yield* Supervisor
