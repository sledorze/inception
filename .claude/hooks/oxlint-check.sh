...
# Pick the tightest config that covers the file: packages/frontend/ has its own
# config that loads the design-system jsPlugin. Forcing the root config here
# silently bypasses that plugin (see hunt 2026-05-15).
lint_config="$repo_root/.oxlintrc.json"
case "$file_path" in
  "$repo_root/packages/frontend/"*) lint_config="$repo_root/packages/frontend/.oxlintrc.json" ;;
esac
